...
class Downloader:
    ...
    def download_chunk(self, session, url, chunk_index, chunk_file_path, start_byte, end_byte, overall_pbar):
    ...
    def download(self, url):
        ...
        if config:
            ...
            if saved_chunk_size_bytes != self.chunk_size_mb * 1024 * 1024 or saved_max_workers != self.max_workers:
                print("\nConfiguration mismatch detected:")
                print(f" - Saved chunk size: {saved_chunk_size_bytes / (1024 * 1024)} MB")
                print(f" - Saved max workers: {saved_max_workers}")
                print(f" - Current chunk size: {self.chunk_size_mb} MB")
                print(f" - Current max workers: {self.max_workers}")
                print("Adjusting to saved configuration...\n")

                self.chunk_size_mb = saved_chunk_size_bytes // (1024 * 1024)
                self.max_workers = saved_max_workers

            print(f"Loaded configuration from state file: url={saved_url}, chunk_size_mb={self.chunk_size_mb}, max_workers={self.max_workers}")
        else:
        ...
        with tqdm(total=file_size, unit="B", unit_scale=True, desc="Overall Progress", position=0, initial=downloaded_size) as overall_pbar:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                for i in range(total_chunks):
                    ...
                    future = executor.submit(self.download_chunk, session, url, i, chunk_file_path, start_byte, end_byte, overall_pbar)
